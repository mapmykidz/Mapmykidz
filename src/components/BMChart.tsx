...
import { interpolateLMS, calculateBMI } from '../utils/calculations'
...
export const BMChart: React.FC<BMChartProps> = ({ results }) => {
  ...
  const chartData = useMemo(() => {
    ...
    const whoData = getWHOBMIData(childData.gender)
    const cdcData = getCDCBMIData(childData.gender)
    ...
    for (let ageMonths = startAge; ageMonths <= endAge; ageMonths += stepSize) {
      ...
      // Calculate BMI values for different percentiles using fixed z-scores
      const calculateBMIForPercentile = (percentile: number): number => {
        let z: number
        switch (percentile) {
          case 3:
          ...
          default:
            ...
            return calculateBMIForPercentile(nearest)
        }
      ...
      const point: ChartDataPoint = {
        ...
        percentile3: calculateBMIForPercentile(3),
        percentile10: calculateBMIForPercentile(10),
        percentile25: calculateBMIForPercentile(25),
        percentile50: calculateBMIForPercentile(50),
        percentile75: calculateBMIForPercentile(75),
        percentile90: calculateBMIForPercentile(90),
        percentile97: calculateBMIForPercentile(97)
      }
  ...
  const childBMI = childData.weight && childData.height ? 
    calculateBMI(
      childData.weightUnit === 'kg' ? childData.weight : childData.weight * 0.45359237,
      childData.heightUnit === 'cm' ? childData.height : childData.height * 2.54
    ) : 0
  ...
  const chartDatasets = {
    ...
    datasets: [
      ...
      // Child's measurement
      {
        label: `${childData.gender === 'male' ? 'Boy' : 'Girl'}'s BMI`,
        data: chartData.map((point, index) => {
          ...
          if (Math.abs(point.age - childAgeYears) < 0.05) {
            ...
            return isFirstMatch ? childBMI : undefined;
          }
          return undefined
        }),
  ...
  const options: ChartOptions<'line'> = {
    ...
    plugins: {
      title: {
        display: true,
        text: `BMI-for-Age Chart - ${childData.gender === 'male' ? 'Boys' : 'Girls'}`,
        font: {
          size: 16,
          weight: 'bold'
        }
      ...
      tooltip: {
        callbacks: {
          ...
          label: (context) => {
            ...
            return `${context.dataset.label}: ${value} kg/m²`
          }
        }
      }
    },
  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-4 text-sm text-gray-600 space-y-2">
        ...
        <p>
          The {childData.gender === 'male' ? 'blue' : 'pink'} dot shows your child's current BMI. 
          Percentile lines show the distribution of BMI values for children of the same age and gender.
        </p>
      </div>
    </div>
  )
